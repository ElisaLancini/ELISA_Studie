%%%%%%%%%%%%%%%%%%%%% (C) Retrieval - Short delay Practice session -
%%%%%%%%%%%%%%%%%%%%% Behavioural %%%%%%%%%%%%%%%%%%%%%%%%e
...
% - Schermata finale che dice "practice test finito" invece di "test finito"
...
input_prompt = {'Randonummer'; 'Condition B';'Session (1=A, 2=B, 3=C, 4=D, 5=E, 6=F)'};
...
%% 3. Load settings
cd(path.input)
C_Settings_retrieval_practice_beh;
...
cd(path.input)
%Load task stimuli
fid=fopen('inputfile_practice.txt','r'); %number of fields
...
breakpoint
    ...
    t_fixation1_offset(i)=Screen('Flip',windowPtr,t_fixation1_onset(i)+fixation_duration-slack);
trigger
breakpoint
     % ---------------- Cue ---------------- %
...
breakpoint
    ...
    t_fixation2_offset(i)= Screen('Flip', windowPtr, t_fixation2_onset(i)+fixation_duration-slack); % show image
trigger
breakpoint
     % ---------------- Selection ---------------- %
    ...
    t_selection_onset(i)= Screen('Flip', windowPtr,  t_fixation2_onset(i)-slack);
    %Record response
    clear KbCheck;  % from kb to buttonbox
    t1 = GetSecs;
    time = 0;
   ...
   timing.fixation1_onset=t_fixation1_onset; % Already in seconds because calculated by PTB
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 13. Save results
     save([path.res num2str(ID) '_' num2str(Session) '.mat']...
        ...
        message = sprintf('Mistakes: %d out of 7 trials ', errornum );
        msgbox(message);
