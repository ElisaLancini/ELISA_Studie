%%%%%%%%%%%%%%%%%%%%% (D) Retrieval - Short delay Practice session - Behavioural %%%%%%%%%%%%%%%%%%%%%%%%
...
% 03.03.2022 Fixed error in "fix1" and "fix 2" for condition A = 2 
%% Inizialising + Part 1
...
ConditionB  = str2num(input_answer{3,1}); % Yes no counterbalancinge
...
t_fixation1_onset=zeros(numTrials,1);
t_fixation1_offset=zeros(numTrials,1);
t_cue_onset=zeros(numTrials,1);
t_cue_offset=zeros(numTrials,1);
t_selection_onset=zeros(numTrials,1);
t_selection_offset=zeros(numTrials,1);
t_classification_onset=zeros(numTrials,1);
t_classification_offset=zeros(numTrials,1);
t_fixation2_onset=zeros(numTrials,1);
t_fixation2_offset=zeros(numTrials,1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% find in rows rand gli 1 e gli 0

% find the line where the stimuli from the first block ended up
...
breakpoint      
        ...
        fixation_texture=Screen('MakeTexture', windowPtr, fixation_pic);
        Screen('DrawTexture', windowPtr, fixation_texture, [], topcentral);
        t_fixation_onset(i)=Screen('Flip',windowPtr,  t_last_onset(i)-slack); trigger_toc(i,14)=toc(startscreen);
        t_fixation_offset(i)=Screen('Flip',windowPtr,t_fixation_onset(i)+fixation1_duration-slack); trigger_toc(i,15)=toc(startscreen);
trigger
breakpoint  
       % ---------------- Cue ---------------- %
        pic_cue=imread([path.sti stimuli_list_ordered{1, 2}{i, 1}], 'png');
        pic_cue_texture=Screen('MakeTexture', windowPtr, pic_cue);
        Screen('DrawTexture', windowPtr, pic_cue_texture, [], topcentral);
        t_cue_onset(i)= Screen('Flip', windowPtr, t_fixation_offset(i)-slack); trigger_toc(i,16)=toc(startscreen);
        t_cue_offset(i)=Screen('Flip', windowPtr, t_cue_onset(i)+ cue_duration-slack); trigger_toc(i,17)=toc(startscreen);
trigger
breakpoint  
        % ---------------- Question "Were there objects?" ---------------- %
        if ConditionB==1
        ...
        t_classification_onset(i)= Screen('Flip', windowPtr, t_cue_offset(i)-slack); trigger_toc(i,18)=toc(startscreen);
        %Record response
    clear KbCheck;  % from kb to buttonbox
        t1 = GetSecs;
        time = 0;
     while time < classification_timeout
        ...
        if (keyIsDown) %has a key been pressed?
         ...
         clear KbCheck;  % from buttonbox to kb
        t_classification_offset(i)= Screen('Flip', windowPtr, t_classification_onset(i)+classification_timeout-slack); trigger_toc(i,19)=toc(startscreen);
trigger
breakpoint       
        % ---------------- Fixation 2 ---------------- %
        fixation_pic=imread(load_fixation, 'png');
        fixation_texture=Screen('MakeTexture', windowPtr, fixation_pic);
        Screen('DrawTexture', windowPtr, fixation_texture, [], topcentral);
        t_fixation2_onset(i)= Screen('Flip', windowPtr, t_classification_offset(i)-slack); trigger_toc(i,20)=toc(startscreen);
        t_fixation2_offset(i)= Screen('Flip', windowPtr, t_fixation2_onset(i)+fixation2_duration-slack); trigger_toc(i,21)=toc(startscreen);
trigger
breakpoint     
        % ---------------- Selection ---------------- %
        % Select which picture to read
        ...
        % Flip (draw all toghether)
        Screen('DrawTextures', windowPtr, pics, [], positions);
        t_selection_onset(i)= Screen('Flip', windowPtr,  t_fixation2_offset(i)-slack); trigger_toc(i,22)=toc(startscreen);
        %Record response
        clear KbCheck;  % from kb to buttonbox
        t1 = GetSecs;
        time = 0;
    ...
    clear KbCheck;  % from buttonbox to kb
      t_selection_offset(i)= Screen('Flip', windowPtr, t_selection_onset(i)+selection_timeout-slack); trigger_toc(i,23)=toc(startscreen);
        time_lastbackup=toc(startscript);
        t_last_onset(i+1)=t_selection_offset(i);        
        % Backup of answers after every keypressed
        save([path.res num2str(ID) '_' num2str(Session) '_backup.mat']);
