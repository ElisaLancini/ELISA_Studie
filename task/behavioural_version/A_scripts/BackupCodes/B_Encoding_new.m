...
%Implementing KbQueue 
KbQueueDevice = -1; %-1 = all keyboard device
activeKeys = {'t','e','c','m','space'};

if ischar(activeKeys) || iscellstr(activeKeys) % key names
        kCode = zeros(256, 1);
        kCode(KbName(activeKeys)) = 1;
elseif length(activeKeys)==256 % full keycode
        kCode = activeKeys;
else
        kCode = zeros(256, 1);
        kCode(activeKeys) = 1;        
...
KbQueueStart(KbQueueDevice);  %starts the cue
...
while 1
    ...
    firstPress(:,[1:68 end 70:83 end 85:length(firstPress)])=NaN; %Selecting only 'e'=69 and 't'=84
    if pressed && length(find(~isnan(firstPress)))>=1
        firstPress(find(firstPress==0))= NaN; %little trick to get rid of 0s
        [secs Index] = min(firstPress); % gets the RT of the first key-press and its ID
...
%breakpoint
    ...
    %----------Check for trigger or end-of-program ------------%
    ...
    if pressed && length(find(~isnan(firstPress)))>=1
       ...
       thekeys = KbName(Index); %converts KeyID to keyname
       disp(thekeys)
       if thekeys == 't' | thekeys == 'e'
          f_trigger(secs,thekeys)
       end
          KbQueueFlush;
    end
...
%FlushEvents('keyDown')
    t2 = GetSecs;
    time = 0;
    while GetSecs < t2+selection_timeout
        % Response keys (optional; for no subject response use empty list)
        %activeKeys = [KbName('c'),KbName('space'),KbName('m')];
        %RestrictKeysForKbCheck(activeKeys);
        %[keyIsDown,t2,keyCode] = KbCheck; %determine state of keyboard
        [ keyIsDown, firstPress ] = KbQueueCheck; %  check if any key was pressed.
        num_of_keys = find(~isnan(firstPress));
        num_of_keys(ismember(num_of_keys,[84,69]))=[]; %excluding t and e
        
        %time = t2-t1 ;
        %Check any key is pressed or not except t and e
            if length(num_of_keys) >= 1  %(keyIsDown) %has a key been pressed?
                firstPress(find(firstPress==0))= NaN; %little trick to get rid of 0s
                firstPress(84)=NaN;firstPress(69)=NaN;
                [t1 Index] = min(firstPress); % gets the RT of the first key-press and its ID
                thekeys = KbName(Index); %converts KeyID to keyname
                %disp(KbName(find(~isnan(firstPress))))
                %key = (find(keyCode));
                %type= class(key);
                time = t2-t1 ;
                if length(num_of_keys) > 1%type ~= 'double' %If two keys pressed simultaneously, then 0
                    response_key(i,1)= 99;
                    response_kbNum(i,1)= 99;
                    response_time(i,1)= 99;
                    break;
            elseif thekeys == 'c' %keyCode(KbName('c'))==1
                    response_key(i,1)= 1; %if a was pressed, 1
                    response_time(i,1) =time;
                    %keypressed = find(keyCode);
                    response_kbNum(i,1)= 2;
                    break;
            elseif thekeys == 'space' %keyCode(KbName('space'))==1
                    response_key(i,1)= 2; %if space was pressed, 2
                    response_time(i,1) =time;
                    response_kbNum(i,1)= 3;
                    break;
            elseif thekeys == 'm' %keyCode(KbName('m'))==1
...
%         while 1
%             % Response keys (optional; for no subject response use empty list)
%             activeKeys = [KbName('c'),KbName('space'),KbName('m'),KbName('e')];
%             RestrictKeysForKbCheck(activeKeys);
%             [keyIsDown,secs,keyCode] = KbCheck;
%             disp(find(keyCode))
%             if keyCode(KbName('c'))==1 || keyCode(KbName('space'))==1 || keyCode(KbName('m'))==1 || keyCode(KbName('e'))==1
%                 break
%             end
%         end
        while 1
            ...
            if pressed
               ...
               thekeys = KbName(Index); %converts KeyID to keyname
               disp(thekeys)
               if thekeys == 'c' || thekeys == 'space' || thekeys == 'm' || thekeys == 'e'
                  break
               end
            end
        end
...
% WAIT FOR TRIGGER (2)
% Implemented kbQueueCheck instead of kbCheck
% while 1
%         activeKeys = [KbName('t')];
%     RestrictKeysForKbCheck(activeKeys);
%     [keyIsDown,secs,keyCode] = KbCheck;
    ...
    while GetSecs < t2+selection_timeout
        ...
        if length(num_of_keys) >= 1  %(keyIsDown) %has a key been pressed?
            ...
            %disp(KbName(find(~isnan(firstPress))))
                ...
                %type= class(key);
                time = t2-t1 ;
                if length(find(~isnan(firstPress))) > 1%type ~= 'double' %If two keys pressed simultaneously, then 0
                    response_key(i,1)= 99;
                    response_kbNum(i,1)= 99;
                    response_time(i,1)= 99;
                    break;
...
%         end
           ...
           while 1
               ...
               if pressed
                   ...
                   if thekeys == 'c' || thekeys == 'space' || thekeys == 'm'
                        break;
                   end
               end
           end
...
slack = Screen('GetFlipInterval', windowPtr)/2; %Calcola quanto tempo ci sta a flippare lo schermo (serve poi per il calcolo del tempo di present)
% Wait for trigger
%Implemented kbQueueCheck instead of kbQueue
% while 1
%         activeKeys = [KbName('t')];
%     RestrictKeysForKbCheck(activeKeys);
%     [keyIsDown,secs,keyCode] = KbCheck;
...
% breakpoint
    ...
    t1 = GetSecs;
    time = 0;
    while time < t2+selection_timeout
        % Response keys (optional; for no subject response use empty list)
        %activeKeys = [KbName('c'),KbName('space'),KbName('m')];
        %RestrictKeysForKbCheck(activeKeys);
        %[keyIsDown,t2,keyCode] = KbCheck; %determine state of keyboard
