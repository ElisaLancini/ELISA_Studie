% Practice during visit 1 (= A practice Encoding )

% 22.02.2022 : In the dialog box 
 %- "Participant number" is replaced with "Participant ID"
 % - "Append an R" is replaced with "Add "2" to the ID"
 % And code changed, not an R but a 2 in case of repetition
 
clearvars
clear KbCheck;

%cd('C:\Users\lancini\Documents\MATLAB\Psychtoolboxe')
%SetupPsychtoolbox
%% 1. Set pathse
...
input_prompt = {'Participant ID';'Session (0= screening)'};
...
if exist([num2str(ID) '_' num2str(Session) '_randinfo.mat']) == 2
    check_prompt = {'(1) Add "2" to the ID / (2) Overwrite /(3) Break'};
...
sca %Close all
        ...
        message = sprintf('Mistakes: %d out of 14 trials ', errornum );
        msgbox(message);
