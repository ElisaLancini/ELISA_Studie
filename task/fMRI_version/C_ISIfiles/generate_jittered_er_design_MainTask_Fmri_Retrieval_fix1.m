function [meanrecipvif, vifs, design_struct] = generate_jittered_er_design_MainTask_Fmri_Retrieval_fix1(varargin)
...
% !!!!!!! These  parameters were changed because my experiment doesn? fit
% the "normal" default experiment here.

event1duration = 14.50;    % duration of all events but not fixation  (13 for encoding YA/OA, 10,5 for retrieval YA)
event2duration = 0;    % write 0 
trialtypes = 2;        % 2 different typs of trials, 1 is task 1 is control task. Here we write 14 so that the program will create 10 randomized ITI for every trialtype (14) , so 140 trialtypes
trialspertype = 40;    % The program will create 10 ITI for each trialtype, so 10*14 = 140
...
ISImin = 1;           % Minimum duration of fixation cross
ISImean = 3;            % Mean duration of fixation cross
ISIstep = .65;          % Keep as it is
ISImax = 5;             % Maximum duration of fixation cross

% Assumptions about TR and scanning params
% Used in plotting and design construction: Will downsample to TR
TR = 2.4; %Scanner parameter ptimal for 7 tesla and LC imaging.
